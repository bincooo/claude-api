...
func LoginFor(baseURL, suffix, proxy string) (string, string, error) {
	...
	for {
		...
		email = em["email"].(string)
		timestamp := em["timestamp"].(float64)
		token, e := partTwo(endpoint, baseURL, proxy, email, timestamp, session)
		if e != nil {
			if strings.Contains(e.Error(), "Rate limited.") {
				return email, "", e
			}
...
// create email
func partOne(suffix, proxy string) (string, map[string]any, *requests.Session, error) {
	if suffix == "" {
	...
	response, session, err := newRequest(5*time.Second, proxy, http.MethodGet, string(ED)+"js/app.js", params, nil)
	if err != nil {
		return "", nil, nil, err
	}
	...
	if len(matchStr) == 0 {
		return "", nil, nil, errors.New("create_email error::not match smailpro_v2_email")
	}
	split := strings.Split(matchStr, ":")
	reg := regexp.MustCompile(split[0] + `:"\w+"`)
	regStrs := reg.FindAllString(response.Text, -1)
	if len(regStrs) < 2 {
		return "", nil, nil, errors.New("create_email error::not match smailpro_v2_email")
	}

	params = map[string]any{
		"id": strings.Replace(strings.Split(regStrs[1], ":\"")[1], "\"", "", -1),
	}
	...
	key, session, err := getKey("", suffix, -1, proxy, session)
	if err != nil {
		return "", nil, nil, err
	}
	...
	if response.StatusCode != 200 {
		return "", nil, nil, errors.New(response.Text)
	}
	...
	if obj["code"].(float64) != 200 {
		return "", nil, nil, errors.New(obj["msg"].(string))
	}
	items := obj["items"].(map[string]any)
	return rapidapiEndpoint + "###" + rapidapiKey, items, session, nil
}

// send_code
func partTwo(endpoint, baseURL, proxy string, email string, timestamp float64, session *requests.Session) (string, error) {
	if rev != "" {
	...
	code, err := partThree(endpoint, email, timestamp, proxy, session)
	if err != nil {
...
// 接收验证码
func partThree(endpoint, email string, timestamp float64, proxy string, session *requests.Session) (string, error) {
	...
	for {
		...
		key, session, err := getKey(email, "", timestamp, proxy, session)
		if err != nil {
			return "", err
		}
		params["key"] = key
...
func getKey(email, suffix string, timestamp float64, proxy string, session *requests.Session) (string, *requests.Session, error) {
